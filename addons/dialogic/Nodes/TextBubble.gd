...
onready var text_top_bar = $TextureRect/TopBar
onready var text_bottom_color_bar = $TextureRect/BottomColorBar

onready var name_label = $NameLabel
onready var name_label_bg = $NameLabel/ColorRect
onready var name_label_small = $NameLabel/SmallNameLabel

onready var next_indicator = $NextIndicatorContainer/NextIndicator

onready var film_name_label = $Film/Name
onready var film_text = $Film/Text
...
func update_name(name: String, color: Color = Color.white, autocolor: bool=false, small_parsed_name: String='') -> void:
	...
	if not name.empty():
		name = Global.dialogNameParsing(name)
		...
		name_label_small.text = small_parsed_name
		
		film_name_label.text = name
		
		text_bottom_color_bar.rect_size = text_bottom_color_bar.rect_min_size
		text_bottom_color_bar.set("modulate", color)
		
		# Alignment
		call_deferred('align_name_label')
		
		if _themeName == "Tsr Default":
			name_label_small.visible = false
			name_label_bg.set('color', color)
			name_label.set('custom_colors/font_color',Color.black)
			text_top_bar.visible = true
			text_top_bar.set('modulate', color)
		else:
			name_label_small.visible = true
...
func clear():
	...
	name_label.text = ""
	name_label_small.text = ""
	$FocusContainer/Block/Text.bbcode_text = ""
...
func update_text(text:String):
	text = Global.dialogParsing(text) #解析对话
	
	var isFilmMode = Global.IsFilmMode
	...
	$FocusContainer/Block/Text.bbcode_text = text
...
func load_theme(theme: ConfigFile):
	_themeName = theme.get_value("settings", "name", "Mod Default")
	...
	name_label_small.set('custom_fonts/font', DialogicUtil.path_fixer_load(theme.get_value('name', 'font_small', 'res://addons/dialogic/Example Assets/Fonts/NameFont.tres')))
	...
	var contentMarginRight = name_padding.x
	if _themeName == "Tsr Default":
		contentMarginRight += 16
	
	name_style.set('content_margin_left', name_padding.x)
	name_style.set('content_margin_right', contentMarginRight)
