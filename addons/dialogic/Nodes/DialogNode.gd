...
var isChapterEnd : bool = false
var isMouseOnMenu: bool = false
...
func load_config_files():
	...
	# Loading saved Theme
	var themeFileName = Dialogic.get_variable("ThemeFileName", "")
	if themeFileName != "":
		theme_file = themeFileName
	
	current_default_theme = theme_file
	current_theme = load_theme(theme_file)
	
	# Loading saved Film Modes
	var isFilmMode = Dialogic.get_variable("IsFilmMode", false)
	if(int(isFilmMode) == 1):
		intoFilmModeTweens()
		
	# Loading saved Focus Modes
	var isFocusMode = Dialogic.get_variable("IsFocusMode", false)
	if(int(isFocusMode) == 1):
		var focusModeAlign = Dialogic.get_variable("FocusModeAlign", "left-top")
		intoFilmModeTweens(true, focusModeAlign)
...
func load_theme(filename):
	Dialogic.set_variable("ThemeFileName", filename)
	...
	if theme.get_value('box', 'portraits_behind_dialog_box', true):
		move_child(get_node("%Portraits"), 0)
	else:
		move_child(get_node("%Portraits"), 1)
		
	var themeName = theme.get_value("settings", "name", "Default")
	
	if(themeName == "Mod Default"):
		$TextBubble/TextureRect/BlurRect.visible = true
		$TextBubble/TextureRect/BottomColorBar.visible = true
	else:
		$TextBubble/TextureRect/BlurRect.visible = false
		$TextBubble/TextureRect/BottomColorBar.visible = false
...
func _process(delta):
	...
	# If Dialog has finished show, then:
#	if(is_state(state.READY)):
#		$FX/CharacterVoice.stop_voice()
	
func _on_HistoryBackButton_pressed():
	HistoryTimeline._on_toggle_history()
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		# 滚轮向上滑动则进入历史记录页面
		if event.button_index == BUTTON_WHEEL_UP && !HistoryTimeline.is_history_open:
			showMenu()
			$MenuAfterEsc.onHistoryClick()
	
	if not Engine.is_editor_hint() and event.is_action_pressed(Dialogic.get_action_button()) and autoPlayMode:
		autoPlayMode = false
		return
	
	if !$MenuAfterEsc.visible and not Engine.is_editor_hint() and event.is_action_pressed(Dialogic.get_action_button()) and (!noSkipMode and !autoPlayMode):
		# 如果不在游戏进程（出现菜单）中则不再处理点击事件了
		if $MenuAfterEsc.visible:
			return
		...
		'dialogic_002':
			...
			if event['character'] == '':# No character found on the event. Skip.
				...
				if event.get('type', 0) == 0 and not portrait_exists(character_data):
					...
					# ADD IT TO THE SCENE
					get_node("%Portraits").add_child(p)
					...
					# z_index
					get_node("%Portraits").move_child(p, get_portrait_z_index_point(event.get('z_index', 0)))
				...
				# LEAVE MODE -------------------------------------------
					...
					if event['character'] == '[All]':
						...
						for p in get_node("%Portraits").get_children():
				...
				# UPDATE MODE -------------------------------------------
					...
					if portrait_exists(character_data):
						for portrait in get_node("%Portraits").get_children():
							if portrait.character_data.get('file', true) == character_data.get('file', false):
								...
								if event.get('change_z_index', false):
									get_node("%Portraits").move_child(portrait, get_portrait_z_index_point(event.get('z_index', 0)))
		...
		'dialogic_030':
			...
			if event['audio'] == 'play' and 'file' in event.keys() and not event['file'].empty():
				...
				if audio == null:
					...
					audio.connect("finished", self, "_on_AudioEvent_finished")
					add_child(audio)
...
func update_name(character) -> void:
	if character.has('name'):
		...
		var small_parsed_name = character["nickname"]
		
		$TextBubble.update_name(parsed_name, character.get('color', Color.white), current_theme.get_value('name', 'auto_color', true), small_parsed_name)
	else:
...
func get_classic_choice_button(label: String):
	...
	# Text
	button.set('custom_fonts/font', DialogicUtil.path_fixer_load(theme.get_value('buttons', 'font', "res://addons/dialogic/Example Assets/Fonts/DefaultFont.tres")))
...
func get_portrait_z_index_point(z_index):
	for i in range(get_node("%Portraits").get_child_count()):
		if get_node("%Portraits").get_child(i).z_index >= z_index:
			return i
	return get_node("%Portraits").get_child_count()
## -----------------------------------------------------------------------------
## 						GLOSSARY POPUP
## -----------------------------------------------------------------------------
...
func resume_state_from_info(state_info):
	...
	for saved_portrait in state_info['portraits']:
		...
		if portrait_exists(character_data):
			...
			get_node("%Portraits").add_child(p)
			get_node("%Portraits").move_child(p, get_portrait_z_index_point(saved_portrait.get('z_index', 0)))
			p.move_to_position(get_character_position(event['position']))
...
func _on_DialogNode_dialogic_signal(event):
	var splitEvent = event.split(",")
	
	for i in splitEvent.size():
		splitEvent[i] = splitEvent[i].lstrip(" ").rstrip(" ")
	
	match splitEvent[0]:
		"Finished":
			var chapterId = splitEvent[1]
			chapterFinished(chapterId)
			return
			
		"BelongsToast":
			var id = splitEvent[1]
			createBelongsToast(id)
		
		"EnterFilmMode":
			Global.IsFilmMode = true
			Dialogic.set_variable("IsFilmMode", 1)
			intoFilmModeTweens()
			return
		"LeaveFilmMode":
			Global.IsFilmMode = false
			Dialogic.set_variable("IsFilmMode", 0)
			leavedFilmModeTweens()
			return
		"EnterFocusMode":
			Global.IsFocusMode = true
			Dialogic.set_variable("IsFocusMode", 1)
			Dialogic.set_variable("FocusModeAlign", splitEvent[1])
			intoFilmModeTweens(true, splitEvent[1])
			return
		"LeaveFocusMode":
			Global.IsFocusMode = false
			Dialogic.set_variable("IsFocusMode", 0)
			leavedFilmModeTweens(true)
			return
		"Save":
			Dialogic.save(Global.slotName)
		"EffectScreenShake":
			emit_signal("_effect_ScreenShake")
			pass
		
# 进入电影模式下的切换动效
func intoFilmModeTweens(isFocus = false, align = "left-center"):
	var interval = 2
	var tween = get_node("TweenFilm")
	
	tween.remove_all()
	
	if $FilmModeTop.scale.y != 139:
		# 黑条动效
		Global.tweenBuilder(tween, $FilmModeTop, "scale:y", 139, interval)
		Global.tweenBuilder(tween, $FilmModeBottom, "position:y", 941, interval)
	
	# 对话框背景消失动效
	tween.interpolate_property($TextBubble/ColorRect, "modulate", 
		Color.white, Color.transparent, 0, Tween.TRANS_EXPO, Tween.EASE_OUT)
	tween.interpolate_property($TextBubble/TextureRect, "modulate", 
		Color.white, Color.transparent, 0, Tween.TRANS_EXPO, Tween.EASE_OUT)
		
	# 透明化原来的消息框
	tween.interpolate_property($TextBubble/TextContainer, "modulate", 
		Color.white, Color.transparent, 0, Tween.TRANS_EXPO, Tween.EASE_OUT)
	tween.interpolate_property($TextBubble/NameLabel, "modulate", 
		Color.white, Color.transparent, 0, Tween.TRANS_EXPO, Tween.EASE_OUT)
		
	# 如果是电影模式则显示电影模式消息框
	if isFocus == false:
		leaveFocusMode()
		tween.interpolate_property($TextBubble/Film, "modulate", 
		Color.transparent, Color.white, 0, Tween.TRANS_EXPO, Tween.EASE_OUT)
		tween.interpolate_property($TextBubble/Film/Name, "rect_position:y",
		1195, 960, interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
		tween.interpolate_property($TextBubble/Film/Text, "rect_position:y",
		1187, 994, interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
	else:
		# 否则进入聚焦电影模式
		tween.interpolate_property($TextBubble/Film, "modulate", 
		Color.white, Color.transparent, 0, Tween.TRANS_EXPO, Tween.EASE_OUT)
		enterFocusMode()
		setFocusTextAlign(align)
	
	tween.start()

# 离开电影模式下的切换动效
func leavedFilmModeTweens(isFocus = false):
	var interval = 2
	var tween = get_node("TweenFilm")
	...
	# 顶部黑条动效
	tween.interpolate_property($FilmModeTop, "scale", 
		Vector2(1, 139), Vector2(1, 1), interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
	
	# 底部黑条动效
	tween.interpolate_property($FilmModeBottom, "position", 
		Vector2(0, 941), Vector2(0, 1080), interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
	...
	if !isFocus:
		tween.interpolate_property($TextBubble/Film/Name, "rect_position:y",
		960, 1195, interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
		tween.interpolate_property($TextBubble/Film/Text, "rect_position:y",
		994, 1187, interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
	else:
		# 否则退出聚焦电影模式
		leaveFocusMode()
	
	# 显示原来的消息框
#	$TextBubble/TextContainer.visible = true
#	$TextBubble/NameLabel.visible = true
	tween.interpolate_property($TextBubble/TextContainer, "modulate", 
		Color.transparent, Color.white, interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
	tween.interpolate_property($TextBubble/NameLabel, "modulate", 
		Color.transparent, Color.white, interval, Tween.TRANS_EXPO, Tween.EASE_OUT)
...
# 设置聚焦电影模式的文字位置
func setFocusTextAlign(align):
	var positionsDict = {
		"left-top": Vector2(120, 199),
		"left-center": Vector2(120, 480),
		"left-bottom": Vector2(120, 821),
	}
	
	var growVerticalDict = {
		"left-top": Control.GROW_DIRECTION_END,
		"left-center": Control.GROW_DIRECTION_BOTH,
		"left-bottom": Control.GROW_DIRECTION_BEGIN,
	}
	
	$TextBubble/FocusContainer/Block.rect_position = positionsDict[align]
	$TextBubble/FocusContainer/Block/Text.grow_vertical = growVerticalDict[align]

# 进入聚焦电影模式下的切换动效
func enterFocusMode():
	$TweenFilm.interpolate_property($TextBubble/FocusContainer/Block/BG, "rect_size:x", 0, 540, 2, Tween.TRANS_EXPO, Tween.EASE_OUT)
	$TweenFilm.interpolate_property($TextBubble/FocusContainer/Block/Right, "rect_size:x", 0, 16, 1, Tween.TRANS_EXPO, Tween.EASE_OUT, 1)
	$TweenFilm.interpolate_property($TextBubble/FocusContainer/Block/Text, "modulate", Color.transparent, Color.white, 1, Tween.TRANS_EXPO, Tween.EASE_OUT, 1)
	
	$TweenFilm.start()

# 离开聚焦电影模式下的切换动效
func leaveFocusMode():
	$TweenFilm.interpolate_property($TextBubble/FocusContainer/Block/BG, "rect_size:x", 540, 0, 2, Tween.TRANS_EXPO, Tween.EASE_OUT)
	$TweenFilm.interpolate_property($TextBubble/FocusContainer/Block/Right, "rect_size:x", 16, 0, 1, Tween.TRANS_EXPO, Tween.EASE_OUT)
	$TweenFilm.interpolate_property($TextBubble/FocusContainer/Block/Text, "modulate", Color.white, Color.transparent, 1, Tween.TRANS_EXPO, Tween.EASE_OUT)
	
	$TweenFilm.start()

# 章节完成，保存，跳转到章节结算界面
# chapterId：类似于 1.01
func chapterFinished(chapterId):
	isPlaying = false
	isChapterEnd = true
	
	Global.saveConfig("Saved", "LatestChapter", chapterId)
	Dialogic.set_variable(str("Chapter", chapterId, "Finished"), "1")
	Dialogic.save(Global.slotName)
	
	var chapterEnd = preload("res://scenes/chapter-end/ChapterEnd.tscn").instance()
	chapterEnd.chapterId = chapterId
#	print(get_node("/root/Main/Menu").get_child_count())
#	get_node("/root/Main/Menu").add_child(chapterEnd)
	self.add_child(chapterEnd)
#	get_tree().change_scene_to(chapterEnd)

# 游戏时间倒计时记录器
func _on_played_time_timer_timeout():
	# 如果玩家没在游玩则返回
	if $MenuAfterEsc.visible:
		return
	
	var chapterPart = Dialogic.get_variable("NowChapterPart", 0)
	var chapter = int(chapterPart.split(".")[0])
	...
	var chapterPlayedTime = int(Dialogic.get_variable(fieldChapterPlayedTime, 0)) + 1
	Dialogic.set_variable(fieldChapterPlayedTime, chapterPlayedTime)
	
	# 为总游玩时间 + 1
	var fieldFullPlayedTime = str("PlayedTime")
	var fullPlayedTime = int(Dialogic.get_variable(fieldFullPlayedTime, 0)) + 1
	Dialogic.set_variable(fieldFullPlayedTime, fullPlayedTime)

func showMenu():
	isPlaying = $MenuAfterEsc.visible
	$MenuAfterEsc.visible = !$MenuAfterEsc.visible
		
	# 将菜单节点放到最后
	var allChildsAmount = self.get_child_count()
	self.move_child($MenuAfterEsc, allChildsAmount)

func createBelongsToast(id):
...
func _notification(what: int) -> void:
	match what:
		MainLoop.NOTIFICATION_WM_GO_BACK_REQUEST:
			# Android back button to show Menu
			showMenu()


func _on_MenuBlock_gui_input(event):
...
func _on_AudioEvent_finished():
	pass


func _on_MenuBlock_mouse_entered():
	isMouseOnMenu = true
	
func _on_MenuBlock_mouse_exited():
	isMouseOnMenu = false
