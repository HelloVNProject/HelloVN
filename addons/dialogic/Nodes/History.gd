...
func _ready():
	...
	title.title = "TR:HISTORY:TITLE"
	title.buttonIcon = load("res://images/icons/left.png")
#	title.get_node("Container/Button").connect("pressed", self, "_on_HistoryBackButton_pressed")
	$HistoryPopup.add_child(title)
...
func add_history_row_event(eventData):
	...
	newHistoryRow.mouse_filter = MOUSE_FILTER_PASS
	...
	if eventData.event_id == 'dialogic_001':
		var text = Global.dialogParsing(tr(eventData.text))
		newHistoryRow.add_history(str(characterPrefix, text), audioData)
	# Character Arrivals
...
func _on_toggle_history(to = null):
	if HistoryPopup.visible == false || to == true:
...
func _on_ScrollHistoryContainer_gui_input(event):
...
func _process(delta):
	if isMouseTouchDown:
		...
		$HistoryPopup/ScrollHistoryContainer.scroll_vertical_fixed(fixedY)
